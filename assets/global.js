...
(function () {
  ...
  // Get the current domain with a leading dot
  const currentDomain = '.' + window.location.hostname

  // Check if the URL contains the ref parameter
  const ref = getQueryParam('ref')

  if (ref) {
    // Set the customer_ref cookie with the whole query string and dynamic domain
    setCookie('customer_ref', ref, 7, currentDomain) // cookie expires in 7 days
  }

  // Function to update order note with cookie value
  function updateOrderNote() {
    // Get the cookie value
    const cookieValue = getCookie('customer_ref')
    ...
    if (cookieValue) {
      ...
      // Send AJAX request to update the order note
      fetch('/cart/update.js', {
        ...
        .then((data) => {
          console.log('Order note updated:', data)
        })
        .catch((error) => {
  ...
  // Wait for the DOM to fully load before updating the order note
